...
// Undefine VERSION macro to avoid conflicts with NetMsgType::VERSION
#ifdef VERSION
#undef VERSION
#endif
...
// BATHRON: fRequestedSporksIDB removed - sporks no longer exist
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman* connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // BATHRON: Spork system removed - no longer need to request sporks
        // Sapling and shielded transactions are always active
    ...
    if (strCommand == NetMsgType::ADDR || strCommand == NetMsgType::ADDRV2) {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                // If we don't have it, ask for it (unless we're still syncing).
                if (!fAlreadyHave && !IsInitialBlockDownload()) {
                    pfrom->AskFor(inv, 2 * 60 * 1000000);
                }
            }
    ...
    // HU Finality Signature (BATHRON quorum-based finality)
    else if (strCommand == NetMsgType::HUSIG) {
        hu::CHuSignature sig;
        vRecv >> sig;

        LogPrint(BCLog::STATE, "Received HUSIG from peer=%d for block %s\n",
                 pfrom->GetId(), sig.blockHash.ToString().substr(0, 16));

        if (hu::huSignalingManager) {
            // R2: score/ban a peer that floods crypto-invalid or malformed finality
            // sigs (an honest relayer never does — it validates before relay). cs_main
            // is taken ONLY on the malicious path, so the normal (valid) path stays
            // lock-free as designed.
            bool fMisbehave = false;
            hu::huSignalingManager->ProcessHuSignature(sig, pfrom, connman, &fMisbehave);
            if (fMisbehave && pfrom) {
                LOCK(cs_main);
                Misbehaving(pfrom->GetId(), 20, "invalid HU finality signature");
            }
        }
        return true;
    }
        ...
        if (std::find(allMessages.begin(), allMessages.end(), strCommand) != allMessages.end()) {
            // BATHRON: Process tier-two messages directly (legacy MN system removed, DMN only)
            {
                // BATHRON: mnodeman.ProcessMessage() removed - legacy masternode system eliminated
                // BATHRON: masternodePayments.ProcessMessageMasternodePayments() removed - no MN payments in BATHRON (coinbase = recycled fees only)
                // BATHRON: sporkManager.ProcessSpork() removed - spork system eliminated
