...
/* BATHRON hardening (audit F3): a plain memset is elidable by the optimiser when the
 * buffer is not read afterwards, leaving the secret nonce k resident in memory. Clear
 * through a volatile pointer so the writes cannot be optimised away. */
static void secp256k1_vrf_memzero(void *ptr, size_t len) {
    volatile unsigned char *p = (volatile unsigned char *)ptr;
    while (len--) { *p++ = 0; }
}
#define memzero(ptr,size) secp256k1_vrf_memzero((ptr),(size))
...
static int vrf_verify(
    ...
    /* BATHRON hardening (audit F1): the ecmult helpers return 0 (without writing their
     * output point) when the scalar is zero or >= group order — i.e. for a malformed
     * proof with s or c out of range. The upstream code ignored these returns and then
     * read the UNINITIALIZED point in ecsub (undefined behaviour). In consensus code that
     * is unacceptable even though the final memcmp would reject w.p. 1-2^-128. Check every
     * return and reject cleanly (honest proofs always have nonzero c and s < q). */
    /* calculate U = s*B - c*Y */
