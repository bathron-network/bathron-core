...
#ifndef BATHRON_BLOCKASSEMBLER_H
#define BATHRON_BLOCKASSEMBLER_H
...
private:
    ...
    // F-HTLC-2 rollover-liveness: reject a package if any special tx in it is
    // PERMANENTLY invalid at this block's height for a height-monotonic-
    // tightening rule (same guard, same height context as ConnectBlock). This
    // is the defense-in-depth backstop to the mempool sweep: a template never
    // contains a special tx ConnectBlock would reject at the same height, so
    // the DMM producer (fTestValidity=false) never builds a doomed block.
    bool TestPackageSpecialHeight(const CTxMemPool::setEntries& package);
...
// Sign a block with the local masternode's ECDSA operator key
bool SignBlockWithMN(CBlock& block);

#endif // BATHRON_BLOCKASSEMBLER_H
