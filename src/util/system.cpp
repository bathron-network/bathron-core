...
const char * const BATHRON_CONF_FILENAME = "bathron.conf";  // BATHRON Chain config file
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bathrond -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\BATHRON
// Windows >= Vista: C:\Users\Username\AppData\Roaming\BATHRON
// Mac: ~/Library/Application Support/BATHRON
// Unix: ~/.bathron
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BATHRON";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BATHRONParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BATHRONParams
    // Mac: ~/Library/Application Support/BATHRONParams
    // Unix: ~/.bathron-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BATHRONParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == nullptr || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "BATHRONParams";
#else
    // Unix
    return pathRet / ".bathron-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/bathron/sapling-spend.params";
        sapling_output = "/usr/share/bathron/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/bathron/sapling-spend.params";
            sapling_output = "/usr/local/share/bathron/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
