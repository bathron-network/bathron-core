...
void CCoinsViewCache::AddCoin(const COutPoint& outpoint, Coin&& coin, bool possible_overwrite) {
    ...
    if (!possible_overwrite) {
        if (!it->second.coin.IsSpent()) {
            // Debug logging for double-add investigation
...
void AddCoins(CCoinsViewCache& cache, const CTransaction& tx, int nHeight, bool check)
{
    ...
    for (size_t i = 0; i < tx.vout.size(); ++i) {
        ...
        cache.AddCoin(out, Coin(tx.vout[i], nHeight, fCoinbase), overwrite);
    }
}
...
{
    // The current best anchor must always exist in the view. A missing anchor
    // here means the local anchor DB is corrupt; substituting an empty tree (the
    // previous behaviour) would silently corrupt the persisted anchor set during
    // reorg. Fail loudly instead, matching upstream Zcash/PIVX. The call is kept
    // out of assert() so its side effect (filling `tree`) survives -DNDEBUG.
    const bool anchorFound = GetSaplingAnchorAt(currentRoot, tree);
    assert(anchorFound);
}
