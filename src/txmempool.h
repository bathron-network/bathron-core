...
#ifndef BATHRON_TXMEMPOOL_H
#define BATHRON_TXMEMPOOL_H
...
public:
    CTxMemPoolEntry(const CTransactionRef& _tx, const CAmount& _nFee,
            ...
            bool _spendsCoinbase, unsigned int nSigOps);
...
public:
    bool operator()(const CTxMemPoolEntry& a, const CTxMemPoolEntry& b) const
    {
        ...
        // Fee-equal tie-break (hard-test campaign #3, part 1): FIFO by entry time, NOT
        // by TXID. BATHRON settlement traffic (LOCK/UNLOCK/TRANSFER_M1/HTLC) is
        // fee-exempt, so it lands at feerate 0 and ties with any fee-exempt spam. The
        // old TXID tie-break was GRINDABLE — an attacker could grind low-TXID 0-fee HTLC
        // spam that deterministically sorts ahead of honest (also fee-exempt) settlement
        // = free, indefinite settlement censorship. Ordering ties by arrival time makes
        // it first-come-first-served: grinding a TXID no longer lets spam jump an
        // already-queued honest tx. (Volume is still unbounded — the structural cap is
        // campaign #3-part-2 / #4; see doc/HARDTEST-CAMPAIGN.md.) GetTime() and the hash
        // are immutable per entry, so this stays a valid strict-weak-ordering key.
        const int64_t t1 = a.GetTime();
        const int64_t t2 = b.GetTime();
        if (t1 != t2) {
            return t1 < t2;
        }
...
private:
    ...
    std::map<CKeyID, uint256> mapProTxOperatorKeyIDs;  // Operator key conflicts
...
public:
    ...
    /**
     * SF2 (B4.8): evict any mempool tx that spends an output of `creatorTx`.
     * Used on reorg when a settlement/HTLC tx is resurrected from a disconnected
     * block: it created a receipt / vault / HTLC output that has just been undone
     * from the committed settlement DB, so a mempool tx spending that output would
     * be block-invalid (Check{Transfer,Unlock,HTLC*} read the committed DB and
     * would not see the output when both are mined in the same block) and could
     * wedge the producer — the same class as the ProReg self-spend stall.
     * Symmetric to removeProTxReferences: called before the re-accept attempt.
     */
    void removeSettlementReferences(const CTransaction& creatorTx, MemPoolRemovalReason reason);
...
private:
    ...
    void removeProTxPubKeyConflicts(const CTransaction& tx, const CPubKey& pubKey);  // BATHRON: ECDSA
