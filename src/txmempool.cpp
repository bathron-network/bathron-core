...
void CTxMemPool::addUncheckedSpecialTx(const CTransaction& tx)
{
    ...
    switch(tx.nType) {
        case CTransaction::TxType::PROREG: {
            ...
            mapProTxOperatorKeyIDs.emplace(pl.pubKeyOperator.GetID(), txid);
            break;
        }
...
void CTxMemPool::removeUncheckedSpecialTx(const CTransaction& tx)
{
    ...
    switch(tx.nType) {
        case CTransaction::TxType::PROREG: {
            ...
            mapProTxOperatorKeyIDs.erase(pl.pubKeyOperator.GetID());
            break;
        }
...
void CTxMemPool::removeForReorg(const CCoinsViewCache *pcoins, unsigned int nMemPoolHeight, int flags)
{
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        // BIP68: a reorg can change the height of a spent coin, so sequence
        // locks satisfied against the old next-block may not be against the
        // new one — re-check alongside nLockTime finality and evict violators
        // (descendants follow below).
        if (!CheckFinalTx(tx, flags) || !CheckSequenceLocks(*this, *tx, flags)) {
            ...
            for (const CTxIn& txin : tx->vin) {
                ...
                // BATHRON Bootstrap: skip maturity for block 1 (NO premine — coinbase=0; frees burn-backed mint/collateral)
                ...
                if (Params().IsRegTestNet() && coin.nHeight <= 1) {
                    ...
                    bSkipMaturity = true;  // Testnet: block-1 outputs immediately spendable (bootstrap)
                }
...
void CTxMemPool::removeForSpecialTxHeightChange(const CCoinsViewCache& view, unsigned int nMemPoolHeight)
{
    AssertLockHeld(cs_main);
    LOCK(cs);
    setEntries txToRemove;
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        const CTransaction& tx = it->GetTx();
        if (tx.IsNormalType()) continue;  // fast path: only special txs carry these rules
        std::string reason;
        if (IsSpecialTxHeightPermanentlyInvalid(tx, view, nMemPoolHeight, reason)) {
            LogPrint(BCLog::MEMPOOL, "removeForSpecialTxHeightChange: evict %s (%s) now permanently invalid at height %u\n",
                     tx.GetHash().ToString(), reason, nMemPoolHeight);
            txToRemove.insert(it);
        }
    }
    setEntries setAllRemoves;
    ...
    RemoveStaged(setAllRemoves, false, MemPoolRemovalReason::UNKNOWN);
}
...
void CTxMemPool::removeProTxPubKeyConflicts(const CTransaction& tx, const CPubKey& pubKey)
{
    CKeyID keyId = pubKey.GetID();
    if (mapProTxOperatorKeyIDs.count(keyId)) {
        const uint256& conflictHash = mapProTxOperatorKeyIDs.at(keyId);
        if (conflictHash != tx.GetHash() && mapTx.count(conflictHash)) {
            removeRecursive(mapTx.find(conflictHash)->GetTx(), MemPoolRemovalReason::CONFLICT);
        }
    }
...
void CTxMemPool::removeSettlementReferences(const CTransaction& creatorTx, MemPoolRemovalReason reason)
{
    LOCK(cs);
    const uint256 creatorHash = creatorTx.GetHash();
    // Evict every mempool tx that spends any output of creatorTx. Iterate to a
    // fixed point because removeRecursive drops descendants too; re-scanning is
    // cheap (outputs are few) and robust against index shifts during removal.
    while (true) {
        const CTransaction* victim = nullptr;
        for (unsigned int i = 0; i < creatorTx.vout.size(); ++i) {
            auto it = mapNextTx.find(COutPoint(creatorHash, i));
            if (it != mapNextTx.end()) { victim = it->second.get(); break; }
        }
        if (!victim) break;
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        if (tx.IsShieldedTx()) {
            for (const SpendDescription& sd : tx.sapData->vShieldedSpend) {
                ...
                // Anchor may not be present during reorg - log instead of crash
                if (!pcoins->GetSaplingAnchorAt(sd.anchor, tree)) {
                    LogPrintf("%s: Sapling anchor %s not found for tx %s\n",
                              __func__, sd.anchor.ToString(), tx.GetHash().ToString());
                }
...
bool CTxMemPool::existsProviderTxConflict(const CTransaction &tx) const
{
    ...
    switch(tx.nType) {
        case CTransaction::TxType::PROREG: {
            ...
            // MULTI-MN v4.0: Operator key AND IP address conflict checks REMOVED
            // One operator can manage N masternodes with a SINGLE key on SAME IP
            // Only owner key must remain unique (collateral protection)
            if (mapProTxPubKeyIDs.count(pl.keyIDOwner)) {
                return true;
            }
            if (!pl.collateralOutpoint.hash.IsNull()) {
                // the ProRegTx must not spend its OWN referenced collateral as an input
                // (mempool-valid but ConnectBlock-invalid → bad-protx-collateral-spent-self,
                // would stall production). Reject on relay.
                for (const CTxIn& in : tx.vin) {
                    if (in.prevout == pl.collateralOutpoint) return true;
                }
