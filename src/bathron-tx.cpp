...
// Copyright (c) 2015-2021 The HU Core developers
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " hu-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  hu-tx [options] <hex-tx> [commands]  Update hex-encoded hu transaction\n" +
                               "or:     hu-tx [options] -create [commands]   Create hex-encoded hu transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded hu transaction
