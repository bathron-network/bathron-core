...
// ═══════════════════════════════════════════════════════════════════════════
// BATHRON Chain Marker - Rejects incompatible datadirs (hard reset protection)
// ═══════════════════════════════════════════════════════════════════════════
// Version 1: Initial M0/M1 settlement layer (January 2026)
// Increment this version to force users to delete old datadirs
static const int BATHRON_CHAIN_VERSION = 1;
static const std::string BATHRON_CHAIN_MARKER = "bathron_chain_version";
 ...
 */
const char * const HU_PID_FILENAME = "hud.pid";  // HU-Core daemon PID file

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", HU_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("bathron-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", BATHRON_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", HU_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf("Fees (in %s/Kb) smaller than this are considered zero fee for relaying and transaction creation (default: %s)", CURRENCY_UNIT, FormatMoney(::minRelayTxFee.GetFeePerK())));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2025-%i The %s Developers"), COPYRIGHT_YEAR, PACKAGE_NAME)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("bathron-loadblk");
    CImportingNow imp;
    ...
    if (fReindex) {
        ...
        // Update money supply after reindex completes
        {
            LOCK(cs_main);
            MoneySupply.Update(pcoinsTip->GetTotalAmount(), chainActive.Height());
            LogPrintf("Money supply updated after reindex: %s at height %d\n",
                      FormatMoney(MoneySupply.Get()), chainActive.Height());
        }
    }
...
/** Sanity checks
 *  Ensure that BATHRON is running in a usable environment with all
...
bool InitNUParams()
{
    if (gArgs.IsArgSet("-nuparams")) {
        // Allow overriding network upgrade parameters for testing — REGTEST ONLY.
        // On a permanent public testnet this must NOT be honored: a -nuparams line in an
        // operator's config would locally move/disable a consensus gate and self-fork that
        // node. (It was briefly widened to testnet for the 2026-06 VRF runtime-gate runs;
        // that gate no longer exists, so the widening is reverted.)
...
#endif
    LogPrintf("BATHRON Chain version %s\n", version_string);
}
...
bool AppInitParameterInteraction()
{
    ...
    // Check level must be 4 for legacy checks
...
{
    // Make sure only a single BATHRON process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BATHRON_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile because if BATHRON is started in the future " /* Continued */
                  "from a different location. It will be unable to locate the current data files. There could " /* Continued */
                  "also be data loss if BATHRON is started while in a temporary directory.\n",
    ...
    // BATHRON: -sporkkey handling removed - spork system eliminated
    ...
    if (gArgs.GetBoolArg("-resync", false)) {
        ...
        LogPrintf("Deleting blockchain folders blocks, chainstate and evodb\n");
        std::vector<fs::path> removeDirs{blocksDir, chainstateDir, evoDir};
        // Delete each folder individually in case one is missing already
        try {
...
// ********************************************************* Step 5: Verify wallet database integrity
    ...
    // Log BATHRON DMM + Finality consensus parameters
    LogPrintf("BATHRON consensus params: quorum_floor=%d, E=%d, lag_warning=%d, leader_timeout=%ds, max_reorg=%d\n",
        consensus.nHuQuorumSize,
        consensus.nHuExpectedCommitteeSize,
        consensus.nHuFinalityLagWarning,
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // BP30 Rebuild-From-Truth: Check if settlement DB is missing BEFORE init
                // (InitSettlementDB creates the directory, so check first)
                bool fSettlementWasMissing = IsSettlementDBMissing();
                if (fSettlementWasMissing) {
                    LogPrintf("Settlement: Database missing at startup (will rebuild after chain load)\n");
                }

                // ═══════════════════════════════════════════════════════════════════════════
                // GENESIS EPOCH CHECK - Auto-wipe stale databases on testnet reset
                // ═══════════════════════════════════════════════════════════════════════════
                // Simple: one file "epoch" in datadir. If mismatch → wipe all DBs.
                // ═══════════════════════════════════════════════════════════════════════════
                bool fEpochWipe = false;
                if (Params().IsTestnet()) {
                    fs::path epochFile = GetDataDir() / "epoch";
                    int storedEpoch = 0;

                    if (fs::exists(epochFile)) {
                        std::ifstream ifs(epochFile.string());
                        ifs >> storedEpoch;
                    }

                    if (storedEpoch != TESTNET_EPOCH) {
                        fEpochWipe = true;
                        LogPrintf("═══════════════════════════════════════════════════════════════════\n");
                        LogPrintf("GENESIS EPOCH RESET: stored=%d, current=%d - wiping all DBs\n",
                                  storedEpoch, TESTNET_EPOCH);
                        LogPrintf("═══════════════════════════════════════════════════════════════════\n");

                        // Write new epoch
                        std::ofstream ofs(epochFile.string());
                        ofs << TESTNET_EPOCH;
                    }
                }
                bool fWipeDBs = fReindex || fEpochWipe;

                // BP30: Initialize Settlement database
                if (!InitSettlementDB(1 << 20, false, fWipeDBs)) { // 1 MB cache
                    UIError(_("Failed to initialize Settlement database"));
                    return false;
                }

                // BP02: Initialize HTLC database
                if (!InitHtlcDB(1 << 20, false, fWipeDBs)) { // 1 MB cache
                    UIError(_("Failed to initialize HTLC database"));
                    return false;
                }
                ...
                if (!InitBurnClaimDB(1 << 20, false, fWipeDBs)) {  // 1 MB cache
                    UIError(_("Failed to initialize burn claim database"));
                    return false;
                }

                // BP11: Daemon-only burn detection
                // All burns (including "genesis" burns) are detected by burn_claim_daemon
                // after network starts. No special genesis files needed.

                // BP12: Initialize kill switch (reads config, defaults to enabled)
                InitKillSwitch();

                // BP-SPVMNPUB: Initialize BTC headers database.
                // Wipe on reindex like every other consensus DB (settlement/htlc/
                // burnclaim all use fWipeDBs above): btcheadersdb is a deterministic
                // projection of the chain. The NEW genesis flow carries ALL BTC
                // headers in block 1's TX_BTC_HEADERS, so a wiped DB is fully rebuilt
                // by replaying from block 1 (no chicken-and-egg). Rebuilding in block
                // order also keeps the active tip monotonic and makes the exact-replay
                // shortcut a no-op on reindex (hardening: removes the load-bearing role
                // that let a duplicate/partial-match tx corrupt the index).
                if (!InitBtcHeadersDB(1 << 20, false, fWipeDBs)) {  // 1 MB cache, wipe on reindex/epoch
                    UIError(_("Failed to initialize BTC headers database"));
                    return false;
                }

                // A1: hook OP_BTCSTATEVERIFY to the consensus header chain and
                // seed the evaluation snapshot (re-anchored at each ConnectBlock).
                InstallBtcStateProvider();
                BtcStateCaptureSnapshot();

                // NOTE: BootstrapBtcHeadersDBFromSPV removed - Block 1 TX_BTC_HEADERS
                // populates btcheadersdb via consensus replay (no pre-distribution needed)
                // NOTE: All burns (including pre-launch) detected by burn_claim_daemon
                ...
                // BATHRON: sporkManager.LoadSporksFromDB() removed - spork system eliminated
                ...
                // ═══════════════════════════════════════════════════════════════════════════
                // BATHRON Chain Marker Check - Reject incompatible datadirs
                // ═══════════════════════════════════════════════════════════════════════════
                // This prevents accidentally using old PIVX/PIV2 datadirs with BATHRON.
                // If the database exists but lacks the marker, it's an incompatible datadir.
                // ═══════════════════════════════════════════════════════════════════════════
                if (!mapBlockIndex.empty()) {
                    int nChainVersion = 0;
                    if (!pblocktree->ReadInt(BATHRON_CHAIN_MARKER, nChainVersion)) {
                        // No marker found - check if genesis block matches BATHRON
                        // This handles migration from pre-marker datadirs
                        auto it = mapBlockIndex.find(chainparams.GetConsensus().hashGenesisBlock);
                        if (it != mapBlockIndex.end() && it->second->nHeight == 0) {
                            LogPrintf("BATHRON chain marker missing but genesis matches - migrating datadir\n");
                            if (!pblocktree->WriteInt(BATHRON_CHAIN_MARKER, BATHRON_CHAIN_VERSION)) {
                                strLoadError = _("Error writing BATHRON chain marker during migration");
                                break;
                            }
                            LogPrintf("BATHRON chain marker written during migration: version %d\n", BATHRON_CHAIN_VERSION);
                        } else {
                            return UIError(_("Incompatible datadir: Missing BATHRON chain marker. "
                                            "This appears to be an old datadir. "
                                            "Please delete it and start fresh, or use -datadir to specify a new location."));
                        }
                    } else if (nChainVersion != BATHRON_CHAIN_VERSION) {
                        return UIError(strprintf(_("Incompatible datadir: Chain version %d found, expected %d. "
                                                  "Please delete the datadir and start fresh."),
                                                nChainVersion, BATHRON_CHAIN_VERSION));
                    } else {
                        LogPrintf("BATHRON chain marker verified: version %d\n", nChainVersion);
                    }
                }
                ...
                bool bWasFreshDatabase = mapBlockIndex.empty();
                if (!fReindex && !LoadGenesisBlock()) {
                    strLoadError = _("Error initializing block database");
                    break;
                }

                // Write BATHRON chain marker for fresh databases OR during reindex
                // During reindex, the marker may be missing even though we have data
                if ((bWasFreshDatabase || fReindex) && !mapBlockIndex.empty()) {
                    int existingVersion = 0;
                    bool hasMarker = pblocktree->ReadInt(BATHRON_CHAIN_MARKER, existingVersion);
                    if (!hasMarker || existingVersion != BATHRON_CHAIN_VERSION) {
                        if (!pblocktree->WriteInt(BATHRON_CHAIN_MARKER, BATHRON_CHAIN_VERSION)) {
                            strLoadError = _("Error writing BATHRON chain marker");
                            break;
                        }
                        LogPrintf("BATHRON chain marker written: version %d (fresh=%d, reindex=%d)\n",
                                  BATHRON_CHAIN_VERSION, bWasFreshDatabase, fReindex);
                    }
                }
                ...
                if (!is_coinsview_empty) {
                    ...
                    if (fSettlementWasMissing) {
                        LogPrintf("Settlement: Database was missing, will rebuild from chain\n");
                        fSettlementRequiresRebuild = true;
                    } else if (!CheckSettlementDBConsistency(chainActive.Tip()->GetBlockHash(),
                                                             chainActive.Tip()->nHeight,
                                                             fSettlementRequiresRebuild)) {
                        if (fSettlementRequiresRebuild) {
                            LogPrintf("Settlement: Database inconsistent, will rebuild from chain\n");
                        }
                    }
                    ...
                    // Ref: doc/BUG-REORG-ATOMICITY-ANALYSIS.md
                    const uint256& chainTipHash = chainActive.Tip()->GetBlockHash();

                    // 1) EvoDB - Masternode list must match chain tip
                    if (evoDb && !evoDb->VerifyBestBlock(chainTipHash)) {
                        strLoadError = strprintf(_("EvoDB is inconsistent with blockchain tip (%s). "
                                                   "Please restart with -reindex to rebuild."),
                                                 chainTipHash.ToString().substr(0, 16));
                        break;
                    }

                    // 2) BurnClaim DB - Burn claim state must match chain tip
                    if (g_burnclaimdb) {
                        bool fBurnClaimRequiresRebuild = false;
                        if (!CheckBurnClaimDBConsistency(chainTipHash, fBurnClaimRequiresRebuild)) {
                            if (fBurnClaimRequiresRebuild) {
                                strLoadError = strprintf(_("BurnClaim database is inconsistent with blockchain tip (%s). "
                                                           "Please restart with -reindex to rebuild."),
                                                         chainTipHash.ToString().substr(0, 16));
                                break;
                            }
                    ...
                    if (g_btcheadersdb) {
                        ...
                        if (!CheckBtcHeadersDBConsistency(chainTipHash, fBtcHeadersRequiresRebuild)) {
                    ...
                    LogPrintf("All consensus DBs consistent with chain tip %s\n",
                              chainTipHash.ToString().substr(0, 16));
                }

                // Fresh genesis - no historical legacy data to prune
...
// ********************************************************* Step 8: Backup and Load wallet
    ...
    // Use 'generate' RPC in regtest for testing
    ...
    LogPrintf("MN-only block production model active\n");
#endif
