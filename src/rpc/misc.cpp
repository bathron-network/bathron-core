...
UniValue mnsync(const JSONRPCRequest& request)
{
    ...
    if (strMode == "status") {
        ...
        // BATHRON: Simplified sync status based on HU finality
        ...
        // BATHRON: Generate sync status inline (no bootstrap phase)
...
// BATHRON: spork RPC removed - see 03-SPORKS-MODERNIZATION blueprint
...
{
public:
    explicit DescribePaymentAddressVisitor(CWallet *_pwallet) : pwallet(_pwallet) {}
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("validateaddress", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\"") +
            HelpExampleCli("validateaddress", "\"ps1ra969yfhvhp73rw5ak2xvtcm9fkuqsnmad7qln79mphhdrst3lwu9vvv03yuyqlh42p42st47qd\"") +
            HelpExampleRpc("validateaddress", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\""));
    ...
    if (isValid) {
        ...
        UniValue detail = boost::apply_visitor(DescribePaymentAddressVisitor(pwallet), finalAddress);
        ret.pushKVs(detail);
    }
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            HelpExampleCli("createmultisig", "2 \"[\\\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\\\",\\\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\\\"]\"") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("createmultisig", "2, \"[\\\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\\\",\\\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\\\"]\""));
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            HelpExampleCli("signmessage", "\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\" \"my message\"") +
            "\nVerify the signature\n" +
            HelpExampleCli("verifymessage", "\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\" \"signature\" \"my message\"") +
            "\nAs json rpc\n" +
            HelpExampleRpc("verifymessage", "\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\", \"signature\", \"my message\""));
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "bathron-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ --------
    ...
    // BATHRON: spork RPC removed - see 03-SPORKS-MODERNIZATION
