...
/**
 * ComputeM0FeeInfo - Calculate M0-only fee for settlement transactions
 *
 * BP30 v2.6: Uses ParseSettlementTx() for robust classification WITHOUT DB lookup.
 * This eliminates heuristics and provides reliable fee calculation even for
 * mempool transactions and during reorgs.
 *
 * For TX_LOCK: fee = M0_in - (vault + M0_change), ignoring M1 receipt output
 * For TX_UNLOCK: fee = M0_in - M0_out (M1 flows are conserved separately)
 * For TX_TRANSFER_M1: fee = M0_in - M0_change (M1 passes through 1:1)
 * For NORMAL: fee = sum(vin) - sum(vout)
 *
 * @param tx          Transaction to analyze
 * @param pcoinsView  Coins view for input lookup (can be nullptr)
 * @param view        Output: populated SettlementTxView with classification and amounts
 * @return true if parsing succeeded (check view.complete for full resolution)
 */
static bool ComputeM0FeeInfo(const CTransaction& tx, CCoinsViewCache* pcoinsView,
                             SettlementTxView& view)
{
    // Use ParseSettlementTx for robust classification
    return ParseSettlementTx(tx, pcoinsView, view);
}
...
void TxToJSON(CWallet* const pwallet, const CTransaction& tx, const CBlockIndex* tip, const CBlockIndex* blockindex, UniValue& entry)
{
    ...
    // BATHRON finality: block is finalized after 12 confirmations
    ...
    if (blockindex && tip) {
        ...
        if (confirmations != -1) {
            ...
            // BATHRON finality - block is finalized if depth >= 12
            finalized = blockindex->nHeight > 0 && (tip->nHeight - blockindex->nHeight >= 12);
        } else {
            entry.pushKV("confirmations", 0);
        }
    }
    entry.pushKV("finalized", finalized);

    // M0 fee info for settlement transactions (and normal TXs)
    // BP30 v2.6: Uses ParseSettlementTx() for robust classification WITHOUT DB lookup
    {
        SettlementTxView view;
        CCoinsViewCache* pcoinsView = pcoinsTip.get();

        if (ComputeM0FeeInfo(tx, pcoinsView, view)) {
            UniValue feeInfo(UniValue::VOBJ);

            // Core classification fields
            feeInfo.pushKV("tx_type", view.txType);
            feeInfo.pushKV("complete", view.complete);

            // M0 amounts
            feeInfo.pushKV("m0_in", view.m0_in);
            feeInfo.pushKV("m0_out", view.m0_out);
            feeInfo.pushKV("m0_fee", view.m0_fee);

            // M1 amounts (only if present)
            if (view.m1_in > 0 || view.m1_out > 0) {
                feeInfo.pushKV("m1_in", view.m1_in);
                feeInfo.pushKV("m1_out", view.m1_out);
            }

            // Vault amounts (only for TX_LOCK/TX_UNLOCK)
            if (view.vault_in > 0 || view.vault_out > 0) {
                feeInfo.pushKV("vault_in", view.vault_in);
                feeInfo.pushKV("vault_out", view.vault_out);
            }

            // Completeness details (only if incomplete)
            if (!view.complete) {
                feeInfo.pushKV("missing_inputs", (int64_t)view.missing_inputs);
                if (view.unclassified_inputs > 0) {
                    feeInfo.pushKV("unclassified_inputs", (int64_t)view.unclassified_inputs);
                }
                if (!view.reason_incomplete.empty()) {
                    feeInfo.pushKV("reason_incomplete", view.reason_incomplete);
                }
            }

            // Fee rate in sat/vB (size is already in entry from TxToUniv)
            if (view.complete && entry.exists("size")) {
                int size = entry["size"].get_int();
                if (size > 0 && view.m0_fee >= 0) {
                    double feeRate = (double)view.m0_fee / size;
                    feeInfo.pushKV("m0_feerate_satvb", feeRate);
                }
            }

            entry.pushKV("m0_fee_info", feeInfo);
        }
    }
}
...
UniValue getrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     {\n"
            "       \"value\" : x.xxx,            (numeric) The value in M0\n"
            "       \"n\" : n,                    (numeric) index\n"
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the address, the value is the M0 amount\n"
            "      ,...\n"
            "    }\n"
