...
#ifndef BATHRON_SAPLING_TRANSACTION_BUILDER_H
#define BATHRON_SAPLING_TRANSACTION_BUILDER_H
...
private:
    ...
    bool saplingProofStub = false;  // Skip Sapling proofs for Sapling testing
#endif
...
public:
    ...
    // Set transaction type (e.g., for special transactions)
    // MUST be called before Build() for special transaction types
    void SetType(CTransaction::TxType _type);

    // Set extra payload (for special transaction types)
    // MUST be called before Build() if the tx type requires payload

#ifdef ENABLE_SAPLING_PROOF_STUB
    // Set Sapling proof stub mode - skip Sapling proofs for testnet Sapling testing
    void SetSaplingProofStub(bool stub) { saplingProofStub = stub; }
    bool IsSaplingProofStub() const { return saplingProofStub; }

    // Add a stub Sapling spend (for Sapling testing without valid witness)
    // Only works when saplingProofStub is set
    void AddSaplingSpendStub(
...
#endif // BATHRON_SAPLING_TRANSACTION_BUILDER_H
