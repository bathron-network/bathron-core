...
#include "consensus/upgrades.h" // for NetworkUpgradeActive()
...
bool CheckTransaction(const CTransaction& tx, CValidationState& state, CAmount& nValueOut)
{
    ...
    // if the tx has shielded data, cannot be a coinbase
    if (tx.IsCoinBase())
        ...
        bool isInitBlockDownload)
{
    ...
    // A coinbase transaction cannot have output descriptions nor shielded spends
    if (tx.IsCoinBase()) {
        if (hasShieldedData)
            return state.DoS(
                    dosLevelPotentiallyRelaxing,
                    error("%s: coinbase has output/spend descriptions", __func__ ),
