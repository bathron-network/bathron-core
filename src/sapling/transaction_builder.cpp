...
#ifdef ENABLE_SAPLING_PROOF_STUB
void TransactionBuilder::AddSaplingSpendStub(
    const libzcash::SaplingExpandedSpendingKey& expsk,
    const libzcash::SaplingNote& note)
{
    // Stub version - no witness validation, creates dummy SpendDescription
    ...
    if (!saplingProofStub) {
        throw std::runtime_error("AddSaplingSpendStub requires saplingProofStub to be set");
    }

    // Add value to balance (like regular spend)
    mtx.sapData->valueBalance += note.value();

    // Create a dummy SpendDescription directly in the transaction
    SpendDescription stubSpend;
    stubSpend.cv.SetNull();  // Commitment to value (zeroed for stub)
    stubSpend.anchor.SetNull();  // Anchor (zeroed for stub)
    stubSpend.rk.SetNull();  // Randomized public key (zeroed for stub)

    // Compute nullifier using a dummy position
    auto nf = note.nullifier(expsk.full_viewing_key(), 0);
    if (nf) {
        stubSpend.nullifier = *nf;
    } else {
        stubSpend.nullifier.SetNull();
    }

    // Zero the proof
    std::memset(stubSpend.zkproof.data(), 0, stubSpend.zkproof.size());

    mtx.sapData->vShieldedSpend.push_back(stubSpend);

    LogPrint(BCLog::SAPLING, "SAPLING-STUB: AddSaplingSpendStub added stub spend (value=%lld nf=%s)\n",
             (long long)note.value(), stubSpend.nullifier.ToString().substr(0, 16).c_str());
}
#endif
...
TransactionBuilderResult TransactionBuilder::ProveAndSign()
{
    ...
    //
#ifdef ENABLE_SAPLING_PROOF_STUB
    // In Sapling stub mode, vShieldedSpend may have entries from AddSaplingSpendStub
    bool hasStubSpends = saplingProofStub && !mtx.sapData->vShieldedSpend.empty();
#else
    bool hasStubSpends = false;
#endif

    if (!spends.empty() || !outputs.empty() || hasStubSpends) {
...
#ifdef ENABLE_SAPLING_PROOF_STUB
        // For stub spends added via AddSaplingSpendStub, zero the spendAuthSig.
        // WARNING: these stub spends carry a ZEROED zkproof and spendAuthSig.
        // Consensus does NOT accept them: sapling_validation.cpp unconditionally
        // calls librustzcash_sapling_check_spend/_output/_final_check, which
        // reject zeroed proofs/sigs. This path exists ONLY for debug/structural
        // flows that bypass consensus verification; a build with this enabled
        // silently produces UNSPENDABLE shielded transactions. Never wire stub
        // acceptance into the consensus path.
        if (hasStubSpends && spends.empty()) {
            for (size_t i = 0; i < mtx.sapData->vShieldedSpend.size(); i++) {
                std::memset(mtx.sapData->vShieldedSpend[i].spendAuthSig.data(), 0,
                            mtx.sapData->vShieldedSpend[i].spendAuthSig.size());
            }
            LogPrint(BCLog::SAPLING, "SAPLING-STUB: ProveAndSign zeroed %zu stub spendAuthSig(s)\n",
                     mtx.sapData->vShieldedSpend.size());
        }
#endif
    ...
    for (int nIn = 0; nIn < (int) mtx.vin.size(); nIn++) {
        ...
        bool signSuccess = ProduceSignature(
            ...
            tIn.scriptPubKey, sigdata, SIGVERSION_SAPLING);
...
TransactionBuilderResult TransactionBuilder::AddDummySignatures()
{
    ...
    for (int nIn = 0; nIn < (int) mtx.vin.size(); nIn++) {
        auto tIn = tIns[nIn];
        SignatureData sigdata;
        if (!ProduceSignature(DummySignatureCreator(keystore), tIn.scriptPubKey, sigdata, SIGVERSION_SAPLING)) {
            return TransactionBuilderResult("Failed to sign transaction");
        } else {
            UpdateTransaction(mtx, nIn, sigdata);
        }
