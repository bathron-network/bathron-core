...
#include "hash.h"
#include "net/net.h" // for CSerializedNetMsg
...
{
    // MULTI-MN: Use first managed MN for MNAUTH (network identity)
    // In multi-MN mode, MNAUTH represents the daemon's "primary" identity.
    // The first registered MN is used as the network identity.
    if (!fMasterNode || !activeMasternodeManager || !activeMasternodeManager->IsReady()) {
        return;
    }

    const CActiveMasternodeInfo* activeMnInfo = activeMasternodeManager->GetInfo();
    if (!activeMnInfo->HasAnyMN()) {
        return;
    }

    // Get first MN's data for MNAUTH
    uint256 proTxHash = activeMnInfo->GetFirstProTxHash();
    if (proTxHash.IsNull()) {
        return;  // No MN found on-chain yet
    }

    CPubKey pubKeyOperator = activeMnInfo->GetFirstPubKeyOperator();
    CKey keyOperator = activeMnInfo->GetFirstKeyOperator();
    ...
    uint256 signHash;
    {
        ...
        if (pnode->nVersion < MNAUTH_NODE_VER_VERSION || nOurNodeVersion < MNAUTH_NODE_VER_VERSION) {
            signHash = ::SerializeHash(std::make_tuple(pubKeyOperator, pnode->receivedMNAuthChallenge, pnode->fInbound));
        } else {
            signHash = ::SerializeHash(std::make_tuple(pubKeyOperator, pnode->receivedMNAuthChallenge, pnode->fInbound, nOurNodeVersion));
        }
    }

    CMNAuth mnauth;
    mnauth.proRegTxHash = proTxHash;
    if (!keyOperator.Sign(signHash, mnauth.sig)) {
        LogPrint(BCLog::NET_MN, "CMNAuth::%s -- Failed to sign MNAUTH, peer=%d\n", __func__, pnode->GetId());
        return;
    }
...
bool CMNAuth::ProcessMessage(CNode* pnode, const std::string& strCommand, CDataStream& vRecv, CConnman& connman, CValidationState& state)
{
    ...
    if (strCommand == NetMsgType::MNAUTH) {
        ...
        // BATHRON: ECDSA signature validation (DER encoded, 64-73 bytes)
        if (mnauth.sig.empty() || mnauth.sig.size() < 64 || mnauth.sig.size() > 73) {
            return state.DoS(100, false, REJECT_INVALID, "invalid mnauth signature size");
        }
        ...
        // BATHRON: ECDSA signature verification
        if (!dmn->pdmnState->pubKeyOperator.Verify(signHash, mnauth.sig)) {
        ...
        // MULTI-MN: Use first managed MN for connection deduplication
        if (!fMasterNode || !activeMasternodeManager || !activeMasternodeManager->IsReady()) {
            return true;
        }
        const CActiveMasternodeInfo* activeMnInfo = activeMasternodeManager->GetInfo();
        ...
        uint256 ourProTxHash = activeMnInfo->GetFirstProTxHash();
        if (ourProTxHash.IsNull()) {
            return true;
        }
        ...
        connman.ForEachNode([&](CNode* pnode2) {
            ...
            if (pnode2->verifiedProRegTxHash == mnauth.proRegTxHash) {
                if (fMasterNode) {
                    // Simple deterministic outbound: lower proTxHash initiates outbound
                    bool weInitiateOutbound = ourProTxHash < mnauth.proRegTxHash;
                    LogPrint(BCLog::NET_MN, "CMNAuth::ProcessMessage -- Masternode %s has already verified as peer %d, weInitiateOutbound=%d. peer=%d\n",
                             mnauth.proRegTxHash.ToString(), pnode2->GetId(), weInitiateOutbound, pnode->GetId());
                    if (weInitiateOutbound) {
                        if (pnode2->fInbound) {
