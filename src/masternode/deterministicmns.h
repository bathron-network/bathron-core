...
#ifndef BATHRON_EVO_DETERMINISTICMNS_H
#define BATHRON_EVO_DETERMINISTICMNS_H
...
public:
    ...
    explicit CDeterministicMNState(const ProRegPL& pl)
    {
        ...
        pubKeyOperator = pl.pubKeyOperator;  // BATHRON: Direct assignment (ECDSA)
        pubKeyVRF = pl.pubKeyVRF;
    ...
    {
        pubKeyOperator = CPubKey();  // BATHRON: Reset ECDSA key
        pubKeyVRF = CPubKey();       // VRF key is part of the operator identity
        addr = CService();
    ...
    void UpdateConfirmedHash(const uint256& _confirmedHash)
    {
...
public:
    ...
    // Returns count of valid AND confirmed MNs (can participate in block production)
    size_t GetConfirmedMNsCount() const
    {
        size_t count = 0;
        for (const auto& p : mnMap) {
            if (!p.second->IsPoSeBanned() && !p.second->pdmnState->confirmedHash.IsNull()) {
                count++;
            }
        }
        return count;
...
public:
    ...
    std::vector<CDeterministicMNCPtr> GetMNsByOperatorKey(const CPubKey& pubKey) const;  // MULTI-MN v4.0: returns ALL MNs with this key
...
public:
    ...
    // TEST-ONLY seam: inject a prebuilt MN list at a block index without
    // processing a real block, and (optionally) set it as the chain tip. Used by
    // the multi-MN HU-finality test fixture (src/test/util/mn_finality_setup.*).
    // NEVER called from production paths.
    void SetListForTesting(const CBlockIndex* pindex, const CDeterministicMNList& mnList, bool asTip);
    ...
    // HU: GetAllQuorumMembers removed - see state/quorum.h for the per-block ECVRF sortition
...
#endif // BATHRON_EVO_DETERMINISTICMNS_H
