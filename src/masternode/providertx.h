...
#ifndef BATHRON_EVO_PROVIDERTX_H
#define BATHRON_EVO_PROVIDERTX_H
...
{
public:
    static const uint16_t CURRENT_VERSION = 3;  // BATHRON: v2 = ECDSA, v3 = + VRF key
...
public:
    ...
    CPubKey pubKeyVRF;                                          // v3+: dedicated ECVRF sortition key (secp256k1)
    CKeyID keyIDVoting;
...
public:
    ...
    SERIALIZE_METHODS(ProRegPL, obj)
    {
        ...
        READWRITE(obj.pubKeyVRF);   // v3 is the only version: VRF key always present
...
{
public:
    static const uint16_t CURRENT_VERSION = 2;  // BATHRON: v2 = ECDSA
...
public:
    ...
    {
        READWRITE(obj.nVersion, obj.proTxHash, obj.addr, obj.inputsHash);
        if (!(s.GetType() & SER_GETHASH)) {
            READWRITE(obj.vchSig);
        }
    }
...
public:
    ...
    uint16_t nMode{0}; // only 0 supported for now
    CPubKey pubKeyOperator;
    CPubKey pubKeyVRF;  // v3+: dedicated ECVRF sortition key (allows rotation)
    CKeyID keyIDVoting;
    CScript scriptPayout;
    uint256 inputsHash; // replay protection
    std::vector<unsigned char> vchSig;
...
public:
    SERIALIZE_METHODS(ProUpRegPL, obj)
    {
        ...
        READWRITE(obj.pubKeyVRF);   // v3 is the only version: VRF key always present
        READWRITE(obj.keyIDVoting, obj.scriptPayout, obj.inputsHash);
        if (!(s.GetType() & SER_GETHASH)) {
            READWRITE(obj.vchSig);
        }
    }
...
#endif // BATHRON_EVO_PROVIDERTX_H
