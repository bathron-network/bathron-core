...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    CKeyID vchAddress = pubkey.GetID();
    {
        ...
        pwallet->MarkDirty();
        pwallet->SetAddressBook(vchAddress, strLabel, AddressBook::AddressBookPurpose::RECEIVE);
...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            ImportAddress(pwallet, dest, strLabel, AddressBook::AddressBookPurpose::RECEIVE);
...
// TODO: Needs further review over the HD flow and multisig import.
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    bool fGood = true;
    {
        ...
        while (file.good()) {
            ...
            // Sapling keys
            // Let's see if the address is a valid spending key
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by BATHRON %s\n", CLIENT_BUILD);
                ...
                if (scannedTime <= GetImportTimestamp(request, now) || results.at(i).exists("error")) {
                    ...
                    result.pushKV("error", JSONRPCError(RPC_MISC_ERROR,
                                                strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                                          ...
                                                          "caused by pruning or data corruption (see bathrond log for details) and could "
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("bip38encrypt", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\" \"mypasphrase\"") +
            HelpExampleRpc("bip38encrypt", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\" \"mypasphrase\""));
