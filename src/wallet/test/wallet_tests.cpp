 ...
 * (where "standard" is defined by direct P2PKH scripts)
 *
...
BOOST_AUTO_TEST_CASE(cached_balances_tests)
{
    ...
    // Add debit tx to mempool so IsSpent recognizes the spent output
    fakeMempoolInsertion(wtxDebit.tx);
    wtxDebit.fInMempool = true;
