...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    const std::string example_address = "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\"";
    ...
    for (auto it = pwallet->NewAddressBookIterator(); it.IsValid(); it.Next()) {
        ...
        if (addrBook.name == label) {
            if (!addrBook.isShielded()) {
                ret.pushKV(EncodeDestination(*it.GetCTxDestKey()), AddressBookDataToJSON(addrBook, false));
            } else {
...
UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("setlabel", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\" \"tabby\"") + HelpExampleRpc("setlabel", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\", \"tabby\""));

    LOCK2(cs_main, pwallet->cs_wallet);

    bool isShielded = false;
    const CWDestination& dest = Standard::DecodeDestination(request.params[0].get_str(), isShielded);
    // Make sure the destination is valid
    if (!Standard::IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    }
...
// NOTE: sendtoaddress REMOVED - use sendmany / shieldsendmany instead
...
UniValue shieldsendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "      \"amount\":amount    (numeric, required) The numeric amount in M0 is the value\n"
                ...
                "                           Those recipients will receive less M0 than you enter in their corresponding amount field.\n"
                ...
                + HelpExampleCli("shieldsendmany",
                                 "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\" '[{\"address\": \"ps1ra969yfhvhp73rw5ak2xvtcm9fkuqsnmad7qln79mphhdrst3lwu9vvv03yuyqlh42p42st47qd\" ,\"amount\": 5.0}]'")
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !request.params.empty())
        throw std::runtime_error(
            ...
            "    [\n"
            "      \"address\",     (string) The address\n"
            "      amount,                 (numeric) The amount in M0\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            HelpExampleCli("signmessage", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\" \"my message\"") +
            "\nVerify the signature\n" +
            HelpExampleCli("verifymessage", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\" \"signature\" \"my message\"") +
            "\nAs json rpc\n" +
            HelpExampleRpc("signmessage", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\", \"my message\""));
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount   (numeric) The total amount in M0 received at this address.\n"

            "\nExamples:\n"
            "\nThe amount from transactions with at least 1 confirmation\n" +
            HelpExampleCli("getreceivedbyaddress", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\"") +
            "\nThe amount including unconfirmed transactions, zero confirmations\n" +
            HelpExampleCli("getreceivedbyaddress", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\" 0") +
            "\nThe amount with at least 6 confirmation, very safe\n" +
            HelpExampleCli("getreceivedbyaddress", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\" 6") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("getreceivedbyaddress", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\", 6"));
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in M0 received for this label.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() > 3))
        throw std::runtime_error(
            ...
            "\nReturns unified wallet balance.\n"
            ...
            "\nResult:\n"
            "{\n"
            "  \"total\": n,           (numeric) Total wallet value (M0 + M0_SHIELD)\n"
            "  \"m0\": n,              (numeric) M0 transparent, available for spending\n"
            "  \"bathron_shield\": n,     (numeric) M0_SHIELD (Sapling)\n"
    ...
    bool fIncludeShielded = paramsSize <= 2 || request.params[2].get_bool();
    ...
    // M0 balances (excludes settlement UTXOs by default)
    CAmount nM0 = pwallet->GetAvailableBalance(filter, true, nMinDepth);
    CAmount nImmature = pwallet->GetImmatureBalance();
    CAmount nLocked = pwallet->GetLockedCoins();  // MN collateral locks

    // M1 balance (M1 Receipts owned by wallet)
    CAmount nM1 = 0;
    if (g_settlementdb) {
        // Get all wallet outputs including settlement
        CWallet::AvailableCoinsFilter settlementFilter;
        settlementFilter.fExcludeSettlement = false;  // Include settlement UTXOs
        settlementFilter.minDepth = nMinDepth;
        std::vector<COutput> vAllCoins;
        pwallet->AvailableCoins(&vAllCoins, nullptr, settlementFilter);

        for (const COutput& out : vAllCoins) {
            COutPoint outpoint(out.tx->GetHash(), out.i);
            if (g_settlementdb->IsM1Receipt(outpoint)) {
                nM1 += out.tx->tx->vout[out.i].nValue;
            }
        }
    }

    // Shielded balance (Sapling)
    CAmount nShield = fIncludeShielded ? pwallet->GetAvailableShieldedBalance() : 0;

    // Total wallet value = M0 + M0_SHIELD (M1 is separate asset)
    CAmount nTotal = nM0 + nShield;

    UniValue result(UniValue::VOBJ);
    result.pushKV("total", ValueFromAmount(nTotal));
    result.pushKV("m0", ValueFromAmount(nM0));
    result.pushKV("m1", ValueFromAmount(nM1));
    result.pushKV("m0_shield", ValueFromAmount(nShield));
 ...
 */
static UniValue legacy_sendmany(CWallet* const pwallet, const UniValue& sendTo, int nMinDepth, std::string comment, const UniValue& subtractFeeFromAmount)
{
    LOCK2(cs_main, pwallet->cs_wallet);
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            "sendmany \"\" {\"address\":amount,...} ( minconf \"comment\" subtract_fee_from )\n"
            "\nSend to multiple destinations. Recipients are transparent or shield addresses.\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The address (either transparent or shield) is the key,\n"
            "                                     the numeric amount in M0 is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            HelpExampleCli("sendmany", "\"\" \"{\\\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\\\":0.01,\\\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\\\":0.02}\"") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n" +
            HelpExampleCli("sendmany", "\"\" \"{\\\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\\\":0.01,\\\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\\\":0.02}\" 6 \"testing\"") +
            ...
            "\nAs a json rpc call\n" +
            HelpExampleRpc("sendmany", "\"\", \"{\\\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\\\":0.01,\\\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\\\":0.02}\", 6, \"testing\"")
        );
    ...
    for (const std::string& key : sendTo.getKeys()) {
        bool isShielded = false;
        Standard::DecodeDestination(key, isShielded);
        if (isShielded) {
            fShieldSend = true;
            break;
        }
    ...
    if (fShieldSend) {
        return ShieldSendManyTo(pwallet, sendTo, comment, "", nMinDepth, subtractFeeFromAmount);
    }

    // All recipients are transparent: use Legacy sendmany t->t
    return legacy_sendmany(pwallet, sendTo, nMinDepth, comment, subtractFeeFromAmount);
}
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            HelpExampleCli("addmultisigaddress", "2 \"[\\\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\\\",\\\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\\\"]\"") +
            "\nAs json rpc call\n" +
            HelpExampleRpc("addmultisigaddress", "2, \"[\\\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\\\",\\\"y7XRqXgz1d8ELErDxtwQPnvfbe2ZcUecka\\\"]\""));
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in M0 received by the address\n"
            ...
            HelpExampleRpc("listreceivedbyaddress", "6, true, true, \"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\""));
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4) throw std::runtime_error(
            "listtransactions ( \"dummy\" count from include_watchonly )\n"
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in M0. This is negative for the 'send' category, and positive\n"
            "                                         for the 'receive' category,\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in M0. This is negative and only available for the \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in M0. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in M0. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in M0\n"
            "  \"confirmations\" : n,     (numeric) The number of confirmations\n"
            "  \"bcconfirmations\" : n,             (numeric) DEPRECATED: Will be removed in a future release\n"
            ...
            "      \"amount\" : x.xxx                  (numeric) The amount in M0\n"
            "      \"label\" : \"label\",              (string) A comment for the address/transaction, if any\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
    ...
    // Calculate fee correctly for all transaction types including pure Sapling
    CAmount nFee = 0;
    if (wtx.IsFromMe(filter)) {
        // Check if this is a pure Sapling transaction (no transparent inputs/outputs)
        bool isPureSapling = wtx.tx->vin.empty() && wtx.tx->vout.empty() &&
                             wtx.tx->hasSaplingData() && wtx.tx->sapData->vShieldedSpend.size() > 0;

        if (isPureSapling) {
            // For pure Sapling transactions, fee = valueBalance (positive = outgoing from shield pool)
            nFee = wtx.tx->sapData->valueBalance;
        } else {
            // Standard calculation: total outputs - total inputs from wallet
            nFee = wtx.tx->GetValueOut() + wtx.tx->GetShieldedValueIn() - nDebit;
        }
    }
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending BATHRON\n"
...
UniValue walletlock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !request.params.empty()) {
        throw std::runtime_error(
            ...
            HelpExampleCli("sendtoaddress", "\"yJYD2bfYYBe6qAojSzMKX949H7QoQifNAo\" 1.0") +
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending funds\n" +
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "     \"feeRate\"           (numeric, optional, default 0=estimate) Set a specific feerate (coin per KB)\n"
    ...
    if (request.params.size() > 1) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(changeAddress))
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid address");
        }
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending funds.\n"
...
UniValue settxfee(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in coin/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total transparent balance of the wallet\n"
            ...
            "  \"autocombine_threshold\": x.xxx,          (numeric) the current autocombine threshold\n"
            "  \"keypoololdest\": xxxxxx,                 (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,                     (numeric) how many new keys are pre-generated (only counts external keys)\n"
            "  \"keypoolsize_hd_internal\": xxxx,         (numeric) how many new keys are pre-generated for internal use (used for change outputs, only appears if the wallet is using this feature, otherwise external keys are used)\n"
            "  \"unlocked_until\": ttt,                   (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx                       (numeric) the transaction fee configuration, set in coin/kB\n"
...
UniValue setautocombinethreshold(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.empty() || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"threshold\": n.nnn,         (numeric) auto-combine threshold\n"
...
UniValue getautocombinethreshold(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !request.params.empty())
        throw std::runtime_error(
            ...
            "{\n"
            "  \"enabled\": true|false,        (boolean) true if auto-combine is enabled, otherwise false\n"
            "  \"threshold\": n.nnn            (numeric) the auto-combine threshold amount in M0\n"
...
{ //  category              name                        actor (function)           okSafe argNames
  //  --------------------- ------------------------    -----------------------    ------ --------
    ...
    { "wallet",             "getbalance",               &getbalance,               false, {"minconf","include_watchonly","include_shield"} },
    ...
    // HU-Core: getstakingstatus removed (no PoS staking in HU)
    ...
    { "wallet",             "listtransactions",         &listtransactions,         false, {"dummy","count","from","include_watchonly"} },
    ...
    { "wallet",             "sendmany",                 &sendmany,                 false, {"dummy","amounts","minconf","comment","subtract_fee_from"} },
    // NOTE: sendtoaddress removed - use sendmany / shieldsendmany instead
