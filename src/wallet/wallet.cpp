...
bool CWallet::IsSpent(const COutPoint& outpoint) const
{
    ...
    for (TxSpends::const_iterator it = range.first; it != range.second; ++it) {
        ...
        if (mit != mapWallet.end()) {
            ...
            if (nDepth > 0  || (nDepth == 0 && !mit->second.isAbandoned() && !fConflicted) )
                return true; // Spent
        }
    }
    return false;
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        // Rewrite wallet to ensure no unencrypted private key remnants
        database->Rewrite();

        // Reload database environment for clean state
...
{
    {
        ...
        if (isSaplingActive) {
            if (pprev->hashFinalSaplingRoot.IsNull() ||
                !pcoinsTip->GetSaplingAnchorAt(pprev->hashFinalSaplingRoot, oldSaplingTree)) {
                // Try to get empty root, if that fails too, use empty tree
                if (!pcoinsTip->GetSaplingAnchorAt(SaplingMerkleTree::empty_root(), oldSaplingTree)) {
                    LogPrintf("%s: Sapling empty root not found, using fresh empty tree\n", __func__);
                    oldSaplingTree = SaplingMerkleTree();
                }
            }
        } else {
            // Not Sapling active or no previous block - use empty tree
            if (!pcoinsTip->GetSaplingAnchorAt(SaplingMerkleTree::empty_root(), oldSaplingTree)) {
                oldSaplingTree = SaplingMerkleTree();
            }
        }
    ...
    if (filter != ISMINE_SPENDABLE_SHIELDED && filter != ISMINE_WATCH_ONLY_SHIELDED) {
        ...
        for (unsigned int i = 0; i < tx->vout.size(); i++) {
            if (!pwallet->IsSpent(hashTx, i)) {
                // BP30: Skip settlement layer UTXOs (Vaults, M1 Receipts)
                // These are tracked separately in getbalance M1 field
                if (g_settlementdb) {
                    COutPoint outpoint(hashTx, i);
                    if (!g_settlementdb->IsM0Standard(outpoint)) {
                        continue;  // Skip Vaults and M1 Receipts
                    }
                }
    ...
    CBlockIndex* ret = nullptr;
    {
        ...
        while (pindex && !fAbortRescan) {
            ...
            if (ReadBlockFromDisk(block, pindex)) {
                ...
                // Sapling
                // Try to get the tree state on the path to the tip of our chain
                // This may fail during rescan after node was offline or during reorg
                if (pindex->pprev) {
                    if (Params().GetConsensus().NetworkUpgradeActive(pindex->pprev->nHeight, Consensus::UPGRADE_V5_0)) {
                        SaplingMerkleTree saplingTree;
                        if (pcoinsTip->GetSaplingAnchorAt(pindex->pprev->hashFinalSaplingRoot, saplingTree)) {
                            // Increment note witness caches
                            ChainTipAdded(pindex, &block, saplingTree);
                        } else {
                            // Anchor not found - skip witness update for this block
                            // This can happen when rescanning after node was offline
                            LogPrintf("%s: Sapling anchor %s not found at height %d, skipping witness update\n",
                                      __func__, pindex->pprev->hashFinalSaplingRoot.ToString(), pindex->nHeight);
                        }
                    }
                }
...
CAmount CWallet::GetAvailableBalance(bool fIncludeExternal, bool fIncludeShielded) const
{
    isminefilter filter;
    if (fIncludeShielded) {
        ...
        for (const auto& entry : mapWallet) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                auto res = CheckOutputAvailability(
                        ...
                        coinControl,
                        fCoinsSelected,
                        coinsFilter.fIncludeExternal,
                        coinsFilter.fIncludeLocked);

                if (!res.available) continue;
                if (coinsFilter.fOnlySpendable && !res.spendable) continue;

                // BP30: Filter out settlement layer UTXOs (Vaults, M1 Receipts)
                if (coinsFilter.fExcludeSettlement && g_settlementdb) {
    ...
    int nMinDepth)
{
    ...
    coinFilter.fIncludeExternal = fIncludeExternal;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-bathron-address
                ...
                for (const auto & coin : setCoins) {
                    ...
                    if (!ProduceSignature(DummySignatureCreator(this), scriptPubKey, sigdata, txNew.GetRequiredSigVersion())) {
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CTransactionRef& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, CAmount nFeePay, bool fIncludeExternal, bool* fTxVoided, int nExtraSize, int nMinDepth)
{
    std::vector<CRecipient> vecSend;
    vecSend.emplace_back(scriptPubKey, nValue, false);
    int nChangePosInOut = -1;
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, nChangePosInOut, strFailReason, coinControl, true, nFeePay, fIncludeExternal, fTxVoided, nExtraSize, nMinDepth);
}
...
std::string CWallet::ParseIntoAddress(const CWDestination& dest, const std::string& purpose) {
    return Standard::EncodeDestination(dest, CChainParams::PUBKEY_ADDRESS);
}
...
bool CReserveKey::GetReservedKey(CPubKey& pubkey, bool _internal)
{
    ...
    if (nIndex == -1) {
        ...
        // Key pool reservation for change addresses.
        uint8_t changeType = internal ? HDChain::ChangeType::INTERNAL : HDChain::ChangeType::EXTERNAL;
        CKeyPool keypool;
...
void CWallet::AutoCombineDust(CConnman* connman)
{
    ...
    for (const auto& entry : mapCoinsByAddress) {
        ...
        // gather the spendable dust outputs to combine
        CCoinControl* coinControl = new CCoinControl();
        CAmount nTotalDustValue = 0;
        for (const COutput& out : vCoins) {
            if (!out.fSpendable)
                continue;
            ...
            nTotalDustValue += out.Value();

            // Combine to the threshold and not way above considering the safety margin.
            if ((nTotalDustValue - (nTotalDustValue / 10)) > nAutoCombineThreshold)
                break;
        ...
        if (vDustCoins.size() <= 1)
            continue;

        std::vector<CRecipient> vecSend;
        const CScript& scriptPubKey = GetScriptForDestination(entry.first);
        vecSend.emplace_back(scriptPubKey, nTotalDustValue, false);
        ...
        vecSend[0].nAmount = nTotalDustValue - (nTotalDustValue / 10);
...
CWallet* CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path)
{
    ...
    // check minimum lock split threshold

    int prev_version = walletInstance->GetVersion();

    // Forced upgrade (BATHRON wallets are HD from first run; the PIVX
    // -legacywallet non-HD escape hatch was removed — no pre-HD history exists)
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun) ||
            (!walletInstance->IsLocked() && prev_version == FEATURE_PRE_SPLIT_KEYPOOL)) {
        if (prev_version <= FEATURE_PRE_BATHRON && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        // Create new HD Wallet — BATHRON wallets are HD from first run
        // (the PIVX -legacywallet pre-HD escape hatch was removed).
...
int CWalletTx::GetBlocksToMaturity() const
{
    ...
    if (Params().IsRegTestNet() && m_confirm.block_height == 0)
        return 0;
    // HU TESTNET EXCEPTION: block-1 coinbase is immediately mature for bootstrap (no premine; value 0)
