...
#ifndef BATHRON_WALLET_WALLET_H
#define BATHRON_WALLET_WALLET_H
...
//! -mintxfee default (0.05 sat/vB - Genesis Fee Policy v1.0)
static const CAmount DEFAULT_TRANSACTION_MINFEE = 50;
//! -maxtxfee default
 ...
 *
 * BATHRON Note: Feature versions adjusted for v0.9.0 (CLIENT_VERSION=90000).
 * As a new chain with fresh genesis, no backward compatibility with legacy wallet needed.
 * Original values (169900/170000) assumed v4.x-5.x CLIENT_VERSION and caused
 * "requires newer version" errors when nMinVersion > CLIENT_VERSION.
 */
enum WalletFeature {
    ...
    FEATURE_PRE_BATHRON = 61000, // inherited version..
    ...
    FEATURE_PRE_SPLIT_KEYPOOL = 80000, // Upgraded to HD SPLIT and can have a pre-split keypool (BATHRON: was 169900)

    FEATURE_SAPLING = 90000, // Upgraded to Saplings key manager. (BATHRON: was 170000)
...
public:
    ...
    //! The public key
    CPubKey vchPubKey;
    //! Whether this keypool entry is in the internal or external keypool.
    uint8_t type;
...
public:
    ...
    struct AvailableCoinsFilter {
        ...
        AvailableCoinsFilter(bool _fIncludeExternal,
                             bool _fOnlySafe,
                ...
                fIncludeExternal(_fIncludeExternal),
        ...
        bool fExcludeSettlement{true}; // Exclude Vaults and M1 Receipts (BP30)
    };
    ...
    /// Get 10000 M0 output and keys which can be used for the Masternode
    ...
    CAmount GetAvailableBalance(bool fIncludeExternal = true, bool fIncludeShielded = true) const;
    ...
    bool CreateTransaction(const std::vector<CRecipient>& vecSend,
        ...
        bool sign = true,
        CAmount nFeePay = 0,
        bool fIncludeExternal = false,
        bool* fTxVoided = nullptr,
        int nExtraSize = 0,
        int nMinDepth = 0);

    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CTransactionRef& tx, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl = nullptr, CAmount nFeePay = 0, bool fIncludeExternal = false, bool* fTxVoided = nullptr, int nExtraSize = 0, int nMinDepth = 0);
