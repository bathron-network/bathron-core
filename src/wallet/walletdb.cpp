...
namespace DBKeys {
    ...
    const std::string LOCK_SPLIT_THRESHOLD{"lockSplitThreshold"};
...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        } else if (strType == DBKeys::LOCK_SPLIT_THRESHOLD) {
            // (MN-only consensus, no PoS staking)
            CAmount dummy;
            ssValue >> dummy;
...
DBErrors WalletBatch::LoadWallet(CWallet* pwallet)
{
    ...
    try {
        ...
        while (!complete) {
            ...
            if (!m_batch.ReadAtCursor(ssKey, ssValue, complete)) {
                if (complete) {
...
void MaybeCompactWalletDB()
{
    ...
    for (CWalletRef pwallet : vpwallets) {
        ...
        if (dbh.nLastFlushed != nUpdateCounter && GetTime() - dbh.nLastWalletUpdate >= 2) {
            if (SQLiteBatch::PeriodicFlush(dbh)) {
...
{
    return SQLiteBatch::Recover(wallet_path, callbackDataIn, recoverKVcallback, out_backup_filename);
}
...
{
    return SQLiteBatch::VerifyEnvironment(wallet_path, errorStr);
}

bool WalletBatch::VerifyDatabaseFile(const fs::path& wallet_path, std::string& warningStr, std::string& errorStr)
{
    return SQLiteBatch::VerifyDatabaseFile(wallet_path, warningStr, errorStr, WalletBatch::Recover);
}
