...
#ifndef BATHRON_CHAIN_H
#define BATHRON_CHAIN_H
...
#include "uint256.h"
#include "util/system.h"  // not used by chain.{h,cpp} itself, but transitively load-bearing
                          // for many consumers (e.g. tx_verify.cpp takes error() through here)
...
enum BlockStatus {
    ...
    //! Parsed, version ok, header sanity ok, 1 <= vtx count <= max, timestamp not in future
