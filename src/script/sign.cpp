...
bool ProduceSignature(const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata, SigVersion sigversion, ScriptError* serror)
{
    CScript script = fromPubKey;
    bool solved = true;
    std::vector<valtype> result;
    txnouttype whichType;
    solved = SignStep(creator, script, result, whichType, sigversion);
    CScript subscript;
    ...
    if (solved && whichType == TX_SCRIPTHASH)
    {
        ...
        solved = solved && SignStep(creator, script, result, whichType, sigversion) && whichType != TX_SCRIPTHASH;
...
bool SignSignature(const CKeyStore &keystore, const CScript& fromPubKey, CMutableTransaction& txTo, unsigned int nIn, const CAmount& amount, int nHashType)
{
    ...
    bool ret = ProduceSignature(creator, fromPubKey, sigdata, txToConst.GetRequiredSigVersion());
...
bool IsSolvable(const CKeyStore& store, const CScript& script)
{
    ...
    SignatureData sigs;
    if (ProduceSignature(creator, script, sigs, SIGVERSION_BASE)) {
