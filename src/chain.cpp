...
const CBlockIndex* CBlockIndex::GetAncestor(int height) const
{
    ...
    while (heightWalk > height) {
        ...
        if (pindexWalk->pskip != nullptr &&
            ...
            // The pskip!=nullptr guard (restored from upstream) matters for
            // transient CBlockIndex objects whose skip pointer was never built
            // — e.g. the dummy tip+1 index in CheckSequenceLocks (BIP68): there
            // pskip is null and we must fall through to the pprev walk instead
