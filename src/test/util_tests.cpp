...
{
    // BATHRON: 1 M0 = 1 satoshi, FormatMoney returns raw integer
    BOOST_CHECK_EQUAL(FormatMoney(0, false), "0");
    BOOST_CHECK_EQUAL(FormatMoney(1, false), "1");
    BOOST_CHECK_EQUAL(FormatMoney(100, false), "100");
    BOOST_CHECK_EQUAL(FormatMoney(1000, false), "1000");
    BOOST_CHECK_EQUAL(FormatMoney(COIN, false), "100000000");  // COIN = 100000000 satoshis
    BOOST_CHECK_EQUAL(FormatMoney(COIN, true), "+100000000");
    BOOST_CHECK_EQUAL(FormatMoney(-COIN, false), "-100000000");
    BOOST_CHECK_EQUAL(FormatMoney(-COIN, true), "-100000000");
    BOOST_CHECK_EQUAL(FormatMoney(123456789, false), "123456789");
}

BOOST_AUTO_TEST_CASE(util_ParseMoney)
{
    // BATHRON: 1 M0 = 1 satoshi, ParseMoney parses raw integers only
    CAmount ret = 0;

    // Valid integer parsing
    BOOST_CHECK(ParseMoney("0", ret));
    BOOST_CHECK_EQUAL(ret, 0);

    BOOST_CHECK(ParseMoney("1", ret));
    BOOST_CHECK_EQUAL(ret, 1);

    BOOST_CHECK(ParseMoney("100", ret));
    BOOST_CHECK_EQUAL(ret, 100);

    BOOST_CHECK(ParseMoney("123456789", ret));
    BOOST_CHECK_EQUAL(ret, 123456789);

    BOOST_CHECK(ParseMoney("100000000", ret));
    BOOST_CHECK_EQUAL(ret, COIN);  // COIN = 100000000

    // Whitespace handling
    BOOST_CHECK(ParseMoney("  1000  ", ret));
    BOOST_CHECK_EQUAL(ret, 1000);

    // Decimals are NOT supported in BATHRON (1 M0 = 1 satoshi)
    BOOST_CHECK(!ParseMoney("0.0", ret));
    BOOST_CHECK(!ParseMoney("1.00", ret));
    BOOST_CHECK(!ParseMoney("0.1", ret));
    BOOST_CHECK(!ParseMoney("12345.6789", ret));

    // Overflow protection (max 18 digits)
    BOOST_CHECK(!ParseMoney("9999999999999999999", ret));
...
BOOST_AUTO_TEST_CASE(test_Capitalize)
{
    ...
    BOOST_CHECK_EQUAL(Capitalize("bathron"), "Bathron");
...
BOOST_AUTO_TEST_CASE(test_validateURL)
{
    ...
    BOOST_CHECK(validateURL("https://foo.bar/"));
    BOOST_CHECK(validateURL("http://bathron.foo.bar"));
    BOOST_CHECK(validateURL("https://foo.bar/bathron"));
    BOOST_CHECK(validateURL("https://foo.bar/bathron/more/"));
    BOOST_CHECK(validateURL("https://142.2.3.1"));
    BOOST_CHECK(validateURL("https://foo_bar.bathron.com"));
