...
// Copyright (c) 2019-2021 The BATHRON Core developers
...
// HaveShieldedRequirements is the consensus gate that stops a shielded spend from
// (a) double-spending a note whose nullifier is already recorded, and (b) using an
// anchor (note-commitment-tree root) that is not known to the view. It is called on
// every acceptance path (ATMP, CheckTxInputs, ConnectBlock). This locks in, at the
// unit level, the double-spend prevention proven live (two nodes sharing a spending
// key each spent the same note; exactly one was mined, the conflicting one rejected).
BOOST_AUTO_TEST_CASE(shielded_requirements_double_spend_and_anchor)
{
    CCoinsViewTest base;
    CCoinsViewCache cache(&base);

    // A shielded spend of a note, anchored at the (always-known) empty root and with
    // a fresh nullifier.
    auto makeShieldedSpend = [](const uint256& nullifier, const uint256& anchor) {
        CMutableTransaction mtx;
        mtx.nVersion = CTransaction::TxVersion::SAPLING;  // IsShieldedTx() requires the Sapling version
        SpendDescription sd;
        sd.nullifier = nullifier;
        sd.anchor = anchor;
        mtx.sapData->vShieldedSpend.push_back(sd);
        return MakeTransactionRef(CTransaction(mtx));
    };

    const uint256 nf = GetRandHash();
    const uint256 emptyAnchor = SaplingMerkleTree::empty_root();  // GetSaplingAnchorAt returns true

    // 1. Fresh note, known anchor -> requirements satisfied.
    CTransactionRef txFresh = makeShieldedSpend(nf, emptyAnchor);
    BOOST_REQUIRE(txFresh->IsShieldedTx());
    BOOST_CHECK(cache.HaveShieldedRequirements(*txFresh));

    // 2. Mark that nullifier as already spent, then a tx spending it again is a
    //    DOUBLE-SPEND and must be rejected.
    cache.SetNullifiers(*txFresh, true);
    BOOST_CHECK(!cache.HaveShieldedRequirements(*txFresh));

    // 3. A spend of a DIFFERENT (fresh) note but referencing an UNKNOWN anchor is
    //    rejected too — you cannot spend against a note-commitment root the chain
    //    never produced (prevents forging membership / spending across a reorg that
    //    abandoned the anchor).
    CTransactionRef txBadAnchor = makeShieldedSpend(GetRandHash(), GetRandHash());
    BOOST_CHECK(!cache.HaveShieldedRequirements(*txBadAnchor));

    // 4. Un-spend the nullifier (as a disconnect/reorg would) -> spendable again.
    cache.SetNullifiers(*txFresh, false);
    BOOST_CHECK(cache.HaveShieldedRequirements(*txFresh));

    // 5. A transparent-only tx has no shielded requirements to satisfy -> always true.
    CMutableTransaction plain;
    plain.vin.emplace_back(CTxIn(COutPoint(GetRandHash(), 0)));
    CTransactionRef txPlain = MakeTransactionRef(CTransaction(plain));
    BOOST_REQUIRE(!txPlain->IsShieldedTx());
    BOOST_CHECK(cache.HaveShieldedRequirements(*txPlain));
}
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            if (randiter % 20 < 2 || coinbase_coins.size() < 10) {
                // HU: don't test for duplicate coinbases as those are not possible due to
...
void CheckAddCoinBase(CAmount base_value, CAmount cache_value, CAmount modify_value, CAmount expected_value, char cache_flags, char expected_flags)
{
    ...
    try {
        ...
        test.cache.AddCoin(OUTPOINT, Coin(std::move(output), 1, false), false);
        test.cache.SelfTest();
        GetCoinsMapEntry(test.cache.map(), result_value, result_flags);
...
BOOST_AUTO_TEST_CASE(ccoins_add)
{
     ...
     *
     * HU: Remove Coinbase argument (ref: https://github.com/BATHRON-Project/BATHRON/pull/1775)
     *
