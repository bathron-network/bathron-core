...
// Copyright (c) 2020 The BATHRON Core developers
...
// B6: every REAL freeze gate (not just UPGRADE_TESTDUMMY) must be inactive
// below its activation height and active at/above it. The fixture has already
// forced all upgrades to NO_ACTIVATION_HEIGHT, so each gate starts DISABLED.
// This is the generic activation-height math the mainnet freeze relies on when
// the gates flip from NO_ACTIVATION to their chosen heights.
BOOST_AUTO_TEST_CASE(realGateActivationHeights)
{
    const Consensus::Params& params = Params().GetConsensus();

    struct Gate { Consensus::UpgradeIndex idx; const char* name; };
    const Gate gates[] = {
        ...
        {Consensus::UPGRADE_OPCAT, "OPCAT"},
        {Consensus::UPGRADE_OUTPUTVALUE, "OUTPUTVALUE"},
        {Consensus::UPGRADE_OUTPUTSCRIPT, "OUTPUTSCRIPT"},
    };

    const int H = 500;
    for (const Gate& g : gates) {
        // Starts disabled (fixture set NO_ACTIVATION_HEIGHT).
        BOOST_CHECK_MESSAGE(NetworkUpgradeState(0, params, g.idx) == UPGRADE_DISABLED, g.name);
        BOOST_CHECK_MESSAGE(!params.NetworkUpgradeActive(1000000, g.idx), g.name);

        // Flip to a real activation height and check the boundary.
        UpdateNetworkUpgradeParameters(g.idx, H);
        BOOST_CHECK_MESSAGE(NetworkUpgradeState(0, params, g.idx) == UPGRADE_PENDING, g.name);
        BOOST_CHECK_MESSAGE(NetworkUpgradeState(H - 1, params, g.idx) == UPGRADE_PENDING, g.name);
        BOOST_CHECK_MESSAGE(NetworkUpgradeState(H, params, g.idx) == UPGRADE_ACTIVE, g.name);
        BOOST_CHECK_MESSAGE(!params.NetworkUpgradeActive(H - 1, g.idx), g.name);   // the flip
        BOOST_CHECK_MESSAGE(params.NetworkUpgradeActive(H, g.idx), g.name);
        BOOST_CHECK_MESSAGE(params.NetworkUpgradeActive(H + 1, g.idx), g.name);

        // ALWAYS_ACTIVE == active from height 0 (the testnet/regtest state today).
        UpdateNetworkUpgradeParameters(g.idx, Consensus::NetworkUpgrade::ALWAYS_ACTIVE);
        BOOST_CHECK_MESSAGE(params.NetworkUpgradeActive(0, g.idx), g.name);

        // Back to disabled for isolation (dtor restores real defaults anyway).
        UpdateNetworkUpgradeParameters(g.idx, Consensus::NetworkUpgrade::NO_ACTIVATION_HEIGHT);
        BOOST_CHECK_MESSAGE(!params.NetworkUpgradeActive(1000000, g.idx), g.name);
    }
}
