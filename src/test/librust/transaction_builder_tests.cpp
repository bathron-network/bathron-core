...
// Copyright (c) 2020-2021 The BATHRON Core developers
...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-M0 in, 0.4 M0_SHIELD out, 0.1 t-M0 fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling)
{
    ...
    // --- 0.4 M0_SHIELD in, 0.299 M0_SHIELD out, 0.1 M0_SHIELD fee, 0.001 M0_SHIELD change (separate output, above dust)
    ...
    // in BATHRON 1 M0 = 1 sat, so the 100000-sat change is above the dust
    // threshold and becomes a separate change output (not folded into the fee)
    ...
    // --- Now try with 1 M0_SHIELD in, 0.5 M0_SHIELD out, 0.1 M0_SHIELD fee, 0.4 M0_SHIELD change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 M0 note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 M0_SHIELD out, 0.1 t-M0 fee
    auto builder = TransactionBuilder(consensusParams);
    ...
    // 0.5 t-M0 out, 0.1 t-M0 fee, 0.59999 M0_SHIELD in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
