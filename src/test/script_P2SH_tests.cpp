...
// SF1: a bare OP_TRUE prevout (BATHRON settlement vault / M1 fee-receipt) must
// be treated as standard-input, else TX_UNLOCK & HTLC claims are mempool-dead
// on mainnet (fRequireStandard=true). Solver() has no OP_TRUE template, so
// without the exemption AreInputsStandard would reject it.
BOOST_AUTO_TEST_CASE(op_true_input_is_standard)
{
    LOCK(cs_main);
    CCoinsView coinsDummy;
    CCoinsViewCache coins(&coinsDummy);
    ...
    txFrom.vout[0].scriptPubKey = CScript() << OP_TRUE;      // settlement vault/receipt
    txFrom.vout[0].nValue = 1000;
    txFrom.vout[1].scriptPubKey = CScript() << OP_2 << OP_3; // unsolvable => non-std
    txFrom.vout[1].nValue = 1000;
    AddCoins(coins, CTransaction(txFrom), 0);

    CMutableTransaction spendOpTrue;
    spendOpTrue.vin.resize(1);
    spendOpTrue.vin[0].prevout = COutPoint(txFrom.GetHash(), 0);  // empty scriptSig
    spendOpTrue.vout.resize(1);
    spendOpTrue.vout[0].nValue = 900;
    BOOST_CHECK(::AreInputsStandard(CTransaction(spendOpTrue), coins));

    // Control: a non-OP_TRUE unsolvable prevout is still rejected (the exemption
    // is exactly OP_TRUE, not "any bare script").
    CMutableTransaction spendOther;
    spendOther.vin.resize(1);
    spendOther.vin[0].prevout = COutPoint(txFrom.GetHash(), 1);
    spendOther.vout.resize(1);
    spendOther.vout[0].nValue = 900;
    BOOST_CHECK(!::AreInputsStandard(CTransaction(spendOther), coins));
}
