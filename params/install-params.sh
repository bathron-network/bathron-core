...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/BATHRONParams"
    else
        PARAMS_DIR="$HOME/.bathron-params"
    fi
fi
...
function install_params {
    ...
    if ! [ -f "$filename" ]
    then
        filename="share/bathron/$filename"
    fi
...
function main() {
    ...
    cat <<EOF
BATHRON - install-params.sh

This script will install the BATHRON (Sapling) zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common BATHRON (Sapling) zkSNARK parameters. Note that it is
